...
static uint32_t msm_eeprom_checksum(struct msm_eeprom_ctrl_t *e_ctrl)
{
	...
	const int MODULE_VENDOR_ID = 0x700;
	...
	if( CheckSum != DataSum ) {
        pr_err("%s HI553 EEPROM AWB CheckSum error for 5100K!\n", __func__);
    } else {
        CDBG("%s HI553 EEPROM AWB CheckSum for 5100K - OK\n", __func__);
	...
	if (CheckSum == 0) {
        ...
        pr_err("%s HI553 EEPROM LSC NOT Supported for 5100K!\n", __func__);
	}
	else if( CheckSum != DataSum ) {
		//LSC data exist, But CheckSum Failed!
        pr_err("%s HI553 EEPROM LSC CheckSum error for 5100K!\n", __func__);
    } else {
        //e_ctrl->is_supported |= 0x20; //LSC bit On
        CDBG("%s HI553 EEPROM LSC CheckSum for 5100K - OK\n", __func__);
