...
/* LGE_CHANGE_S. this code is for VT mode fps. 2015-06-05. sangwoo25.park@lge.com */
...
/* LGE_CHANGE_E. this code is for VT mode fps. 2015-06-05. sangwoo25.park@lge.com */

// LGE_CHANGE
static struct msm_camera_i2c_conf_array hi351_prev_conf_in_case_of_init_QR_Normal[] = {
	{&hi351_prev_settings_in_case_of_QR_Normal[HI351_60HZ][0],
	ARRAY_SIZE(hi351_prev_settings_in_case_of_QR_Normal[HI351_60HZ]), 0, MSM_CAMERA_I2C_BYTE_DATA},
	{&hi351_prev_settings_in_case_of_QR_Normal[HI351_50HZ][0],
	ARRAY_SIZE(hi351_prev_settings_in_case_of_QR_Normal[HI351_50HZ]), 0, MSM_CAMERA_I2C_BYTE_DATA},
};
static struct msm_camera_i2c_conf_array hi351_init_conf_QR_Normal[] = {
	{&hi351_recommend_settings_sunny[HI351_60HZ][0],
	ARRAY_SIZE(hi351_recommend_settings_sunny[HI351_60HZ]), 0, MSM_CAMERA_I2C_BYTE_DATA},
	{&hi351_recommend_settings_sunny[HI351_50HZ][0],
	ARRAY_SIZE(hi351_recommend_settings_sunny[HI351_50HZ]), 0, MSM_CAMERA_I2C_BYTE_DATA},
	{&hi351_recommend_settings_cowell_QR_Normal[HI351_60HZ][0],
	ARRAY_SIZE(hi351_recommend_settings_cowell_QR_Normal[HI351_60HZ]), 0, MSM_CAMERA_I2C_BYTE_DATA},
	{&hi351_recommend_settings_cowell_QR_Normal[HI351_50HZ][0],
	ARRAY_SIZE(hi351_recommend_settings_cowell_QR_Normal[HI351_50HZ]), 0, MSM_CAMERA_I2C_BYTE_DATA},
};
...
static int32_t hi351_sensor_match_id(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	pr_info("%s : set default cowell settings\n", __func__);
	return rc;
}
...
// LGE_CHANGE_E. youngwook.song@lge.com, this code is for distinguishing Camera/Camcoder init. from other enterings. 2014-01-21
	...
	//LGE-CHANGE
	case CFG_SET_INIT_SETTING_QR_CODE:{
		int32_t rc = 0;
		pr_err("%s, CFG_SET_INIT_SETTING_QR_CODE!!\n", __func__);
		rc = hi351_sensor_write_init_settings(s_ctrl->sensor_i2c_client,
					(struct msm_camera_i2c_reg_conf *) hi351_init_conf_QR_Normal[hi351_ab_mod].conf, hi351_init_conf_QR_Normal[hi351_ab_mod].size);

		if( INIT_DONE == 0 ) INIT_DONE = 1;
		break;
	}
	case CFG_SET_START_STREAM_QR_CODE:{
		pr_err("%s, CFG_SET_START_STREAM_QR_CODE!! INIT_DONE %d\n", __func__,INIT_DONE);
		if(INIT_DONE == 1){
			hi351_i2c_write_table(s_ctrl, &hi351_start_settings_in_case_of_QR_Normal[0], ARRAY_SIZE(hi351_start_settings_in_case_of_QR_Normal));
		...
		pr_err("%s, CFG_SET_START_STREAM_QR_CODE End!!\n", __func__);
		break;
	}
	case CFG_SET_RESOLUTION_QR_CODE: {
			if(INIT_DONE == 1){
						hi351_sensor_write_init_settings(s_ctrl->sensor_i2c_client,
									(struct msm_camera_i2c_reg_conf *) hi351_prev_conf_in_case_of_init_QR_Normal[hi351_antibanding].conf, hi351_prev_conf_in_case_of_init_QR_Normal[hi351_antibanding].size);
			pr_err("%s, CFG_SET_RESOLUTION_QR_CODE prevsettings followed by INIT!!\n", __func__);
				}
			// conf1 is normal situation.
			else if(INIT_DONE == 0){
						hi351_sensor_write_init_settings(s_ctrl->sensor_i2c_client,
									(struct msm_camera_i2c_reg_conf *) hi351_prev_conf[hi351_antibanding].conf, hi351_prev_conf[hi351_antibanding].size);
			pr_err("%s, CFG_SET_RESOLUTION_QR_CODE prevsettings!!\n", __func__);
				}
		break;
	}
