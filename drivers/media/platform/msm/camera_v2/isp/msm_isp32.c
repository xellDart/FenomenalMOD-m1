...
#define VFE32_EQUAL_SLICE_UB_32KB 286 //302 /* (UB_SIZE - STATS SIZE)/6 */
...
#define MSM_ISP32_TOTAL_WM_UB 792 /*792 double word*/
#define MSM_ISP32_TOTAL_WM_UB_32KB 1716 //1812 /* UB_SIZE - STATS SIZE */
/*792 double word*/
...
static void msm_vfe32_cfg_axi_ub_equal_default(struct vfe_device *vfe_dev)
{
	...
	uint32_t wm_ub_size;
	uint64_t delta;
	uint32_t total_wm_ub = MSM_ISP32_TOTAL_WM_UB;
	...
	if (vfe_dev->vfe_hw_version == VFE32_8909_VERSION)
		total_wm_ub = MSM_ISP32_TOTAL_WM_UB_32KB;
	...
	for (i = 0; i < axi_data->hw_info->num_wm; i++) {
		if (axi_data->free_wm[i]) {
			...
			wm_ub_size = axi_data->hw_info->min_wm_ub +
				(uint32_t)delta;
			pr_err("__debug__: i = %d, ub_offset = %u, wm_ub_size = %u\n", i, ub_offset, wm_ub_size);
...
static void msm_vfe32_cfg_axi_ub_equal_slicing(struct vfe_device *vfe_dev)
{
	...
	uint32_t equal_slice_ub = VFE32_EQUAL_SLICE_UB;
	uint32_t total_wm_ub = VFE32_AXI_SLICE_UB;
	struct msm_vfe_axi_shared_data *axi_data = &vfe_dev->axi_data;
	if (vfe_dev->vfe_hw_version == VFE32_8909_VERSION) {
		equal_slice_ub = VFE32_EQUAL_SLICE_UB_32KB;
		total_wm_ub = MSM_ISP32_TOTAL_WM_UB_32KB;
	}

	for (i = 0; i < axi_data->hw_info->num_wm; i++) {
		if (ub_offset + equal_slice_ub > total_wm_ub) {
			final_ub_slice_size = total_wm_ub - ub_offset;
			msm_camera_io_w(ub_offset << 16 |
			...
			msm_camera_io_w(ub_offset << 16 |
				(equal_slice_ub - 1), vfe_dev->vfe_base +
				VFE32_WM_BASE(i) + 0xC);
