				...
				union lge_power_propval *val)
{
	...
	case LGE_POWER_PROP_XO_THERM_PHY:
		if (!IS_ERR(chip->vadc_dev)) {
			if (chip->xo_therm_channel != 0xFF) {
				rc = qpnp_vadc_read(chip->vadc_dev, chip->xo_therm_channel, &results);
			} else {
	...
	case LGE_POWER_PROP_BATT_THERM_PHY:
		if (!IS_ERR(chip->vadc_dev)) {
			if (chip->batt_therm_channel != 0xFF) {
				rc = qpnp_vadc_read(chip->vadc_dev, chip->batt_therm_channel, &results);
			} else {
	...
	case LGE_POWER_PROP_BATT_THERM_RAW:
		if (!IS_ERR(chip->vadc_dev)) {
			...
			if (rc) {
				pr_err("[LG_CHG] Unable to read battery temperature adc!!!\n");
				ret_val = -EINVAL;
			} else {
				val->int64val = results.adc_code;
			}
	...
	case LGE_POWER_PROP_USB_ID_PHY:
		if (!IS_ERR(chip->vadc_dev)) {
			if (chip->usb_id_channel != 0xFF) {
				rc = qpnp_vadc_read(chip->vadc_dev, chip->usb_id_channel, &results);
			} else {
	...
	case LGE_POWER_PROP_PA_THERM_PHY:
		if (!IS_ERR(chip->vadc_dev)) {
			if (chip->pa_therm_channel != 0xFF) {
				rc = qpnp_vadc_read(chip->vadc_dev, chip->pa_therm_channel, &results);
			} else {
				pr_err("[LG_CHG] VADC is not used for PA_THERM!!!\n");
				rc = -1;
			}
			if (rc) {
				pr_err("[LG_CHG] Unable to read pa temperature adc!!!\n");
				ret_val = -EINVAL;
			} else {
				val->int64val = results.physical;
			}
...
static int lge_adc_qct_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(lge_adc_chip->vadc_dev)) {
		...
		if (ret != -EPROBE_DEFER)
			...
			pr_err("[LGE_CHG] probe defer due to not initializing vadc\n");
		goto err_free;
	}
	...
	pr_info("[LG_CHG][Channel] XO : %d, PA : %d, BATT_THERM : %d, USB_ID : %d\n",
			lge_adc_chip->xo_therm_channel, lge_adc_chip->pa_therm_channel,
			lge_adc_chip->batt_therm_channel, lge_adc_chip->usb_id_channel);
