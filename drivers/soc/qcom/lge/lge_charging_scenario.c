...
extern int get_backlight_state(void);
extern int get_cur_main_lcd_level(void);
#if defined(CONFIG_MACH_MSM8909_M1_SPR_US) || defined(CONFIG_MACH_MSM8909_M1_TRF_US_VZW)
#define LCD_ON_CHG_CURRENT_WITH_HIGH_BRIGHTNESS 450
#define LCD_ON_CHG_CURRENT_WITH_LOW_BRIGHTNESS 540
#define BRIGHTNESS_TO_DETERMINE_CHG_CURRENT 740
#elif defined(CONFIG_MACH_MSM8909_M1_MPCS_US) || defined(CONFIG_MACH_MSM8909_M1_TMO_US)
#define LCD_ON_CHG_CURRENT_WITH_HIGH_BRIGHTNESS 810
#define LCD_ON_CHG_CURRENT_WITH_LOW_BRIGHTNESS 810
#define BRIGHTNESS_TO_DETERMINE_CHG_CURRENT 790
#else
#define LCD_ON_CHG_CURRENT_WITH_HIGH_BRIGHTNESS 630
#define LCD_ON_CHG_CURRENT_WITH_LOW_BRIGHTNESS 630
#define BRIGHTNESS_TO_DETERMINE_CHG_CURRENT 790
#endif
#endif
...
#ifdef CONFIG_LGE_PM_BACKLIGHT_CHG_CONTROL
	if(req.charger_type == POWER_SUPPLY_TYPE_USB_DCP) {
		if (req.chg_current_te < req.chg_current_ma) {
			if(get_backlight_state() && \
				(req.chg_current_te > LCD_ON_CHG_CURRENT_WITH_HIGH_BRIGHTNESS)) {
				if(BRIGHTNESS_TO_DETERMINE_CHG_CURRENT < get_cur_main_lcd_level())
					res->dc_current = LCD_ON_CHG_CURRENT_WITH_HIGH_BRIGHTNESS;
				else
					res->dc_current = LCD_ON_CHG_CURRENT_WITH_LOW_BRIGHTNESS;
			} else
				res->dc_current = req.chg_current_te;
		} else {
			if(get_backlight_state() && \
				(BRIGHTNESS_TO_DETERMINE_CHG_CURRENT < get_cur_main_lcd_level()))
				res->dc_current = LCD_ON_CHG_CURRENT_WITH_HIGH_BRIGHTNESS;
			else if(get_backlight_state() && \
				(BRIGHTNESS_TO_DETERMINE_CHG_CURRENT >= get_cur_main_lcd_level()))
				res->dc_current = LCD_ON_CHG_CURRENT_WITH_LOW_BRIGHTNESS;
			else
