 ...
 ******************************************************************************
 ...
 LGE use schedule_delayed_work for polling
 ******************************************************************************/
...
#define LGE_CHECK_INTERRUPT_1


/* LPSR Feature defines */
#ifdef LGE_LPSR
#define DEFAULT_INT_THR	0x0a
#define DEFAULT_INT_DUR	0x08
/* MAX MD SAMPLE COUNT */
#define MAX_MD_SAMPLE_COUNT 15
#define MD_REPORT_INTERVAL_MS 66
#define LGE_LPSR_NAME		"lge_lpsr"
#endif
...
#ifdef K2HH_ACCEL_CALIBRATION
#define K2HH_SHAKING_DETECT_THRESHOLD 1311	/* eq. to 160  mg */
#define CALIBRATION_DATA_AMOUNT 10
#define TESTLIMIT_XY            (1762)		/* eq. to 215  mg */
#define TESTLIMIT_Z_USL_LSB     (10410)		/* eq. to 1270 mg */
#define TESTLIMIT_Z_LSL_LSB     (6378)		/* eq. to 778  mg */
#define G_RAW_AT_4G		(8192)
#endif
...
#define CTRL2_DFC_ODR_9		(0x40)
#define CTRL2_DFC_ODR_50	(0x00)
#define CTRL2_DFC_ODR_100	(0x20)
#define CTRL2_DFC_ODR_400	(0x60)

/* CTRL3 */
#define CTRL3_IG1_INT1		(0x08)
#define CTRL3_ACT_INT1		(0x20)
#define CTRL3_DRDY_INT1		(0x01)
...
/* INT CFG 1*/
#define INT_CFG1_ZHIE		(0x20)
#define INT_CFG1_ZLIE		(0x10)
#define INT_CFG1_YHIE		(0x08)
#define INT_CFG1_YLIE		(0x04)
#define INT_CFG1_XHIE		(0x02)
#define INT_CFG1_XLIE		(0x01)
...
#define INIT_REPORT_DELAY_MS	10

/* enable_state */
#define ENABLE_NORMAL	0x01
#define ENABLE_MD	0x02
...
struct k2hh_acc_data {
	...
	struct mutex lock;
	struct delayed_work input_poll_work;

#ifdef LGE_LPSR
	struct delayed_work input_md_poll_work;
	struct input_dev *input_dev_md;
	struct mutex lpsr_lock;
#endif

	struct input_dev *input_dev;
#ifdef K2HH_ACCEL_CALIBRATION
	struct k2hh_acc cal_data;
	struct k2hh_acc old_cal_data;
#endif
	int hw_initialized;
	...
	int hw_working;
	atomic_t enabled;
	atomic_t enable_state;
#ifdef LGE_LPSR
	atomic_t md_sample_count;
#endif
	bool int_check;
...
static void k2hh_acc_set_init_register_values(struct k2hh_acc_data *acc)
{
	...
	/*We need to disable hr mode*/
	acc->resume_state[RES_CTRL1] = (ALL_ZEROES | CTRL1_BDU_ENABLE | ACC_ENABLE_ALL_AXES);
	/* Low-pass cutoff frequency in high resolution mode */

	acc->resume_state[RES_CTRL2] = ALL_ZEROES | CTRL2_DFC_ODR_400;
	...
	acc->resume_state[RES_CTRL5] = (ALL_ZEROES | CTRL5_HLACTIVE_H | CTRL5_PPOD_OD);
#ifdef USE_ACC_IRQ2
...
#ifdef LGE_LPSR
	acc->resume_state[RES_INT_THSX1] = acc->pdata->int_thr_x;
	acc->resume_state[RES_INT_THSY1] = acc->pdata->int_thr_y;
	acc->resume_state[RES_INT_THSZ1] = acc->pdata->int_thr_z;
	acc->resume_state[RES_INT_DUR1] = acc->pdata->int_dur;
#endif
}
...
#ifdef LGE_LPSR
static void k2hh_acc_report_values_md(struct k2hh_acc_data *acc,
					int *xyz)
{
	input_report_abs(acc->input_dev_md, ABS_X, xyz[0]);
	input_report_abs(acc->input_dev_md, ABS_Y, xyz[1]);
	input_report_abs(acc->input_dev_md, ABS_Z, xyz[2]);
	input_sync(acc->input_dev_md);
}

static void k2hh_acc_report_triple_md(struct k2hh_acc_data *acc)
{
	int err;
	int xyz[3];
	...
	if (err < 0)
		...
		k2hh_acc_report_values_md(acc, xyz);
}
#endif

#ifdef K2HH_ACCEL_CALIBRATION
static int k2hh_read_old_Calibration_data(struct i2c_client *client)
{
	int fd_offset;
	char offset_src[16];
	...
	fd_offset = sys_open("/sns/old_offset.dat",O_RDONLY, 0);
	...
	ret = sscanf(offset_src,"%ld,%ld,%ld",&offset_x,&offset_y,&offset_z);
	if(3 != ret){
		sys_close(fd_offset);
		return -EINVAL;
	}
	...
	k2hh->old_cal_data.x = offset_x;
	k2hh->old_cal_data.y = offset_y;
	k2hh->old_cal_data.z = offset_z;
...
static void k2hh_acc_irq1_work_func(struct work_struct *work)
{
	...
	/* ; */
	u8 data;
#ifdef LGE_CHECK_INTERRUPT_1
	u8 buf[2];
	if (acc->int_check == false) { /* this routine is done in selftest only */
		data = STATUS;
		k2hh_acc_i2c_read(acc, &data, 1);
		pr_info("%s: int1 oneshot check: %x\n", K2HH_ACC_DEV_NAME, data);
		buf[0]= CTRL3;
		buf[1]= acc->resume_state[RES_CTRL3];
		k2hh_acc_i2c_write(acc, buf, 1);
		acc->int_check = true;
		data = 0;
	}
#endif
	...
	mutex_lock(&acc->lpsr_lock);
	if ((data & 0x40) && atomic_read(&acc->md_sample_count) <= 0) {
		pr_info("%s: Start MD Poller\n", K2HH_ACC_DEV_NAME);
		atomic_set(&acc->md_sample_count, MAX_MD_SAMPLE_COUNT);
		schedule_delayed_work(&acc->input_md_poll_work,
				msecs_to_jiffies(MD_REPORT_INTERVAL_MS));
	} else if (data & 0x40) {
		atomic_set(&acc->md_sample_count, MAX_MD_SAMPLE_COUNT);
	} else {
		pr_info("%s: supurious interrupt\n", K2HH_ACC_DEV_NAME);
	}
	mutex_unlock(&acc->lpsr_lock);
...
static int k2hh_acc_enable(struct k2hh_acc_data *acc)
{
	...
	//pr_info("K2HH old_cal_data.bCalLoaded = %d\n",acc->old_cal_data.bCalLoaded);
	if( !(acc->old_cal_data.bCalLoaded ) ) {
		err = k2hh_read_old_Calibration_data(acc->client);
		if (err)
			pr_info("k2hh old Calibration data is not found\n");
		else
			acc->old_cal_data.bCalLoaded = 1;
	}
	//pr_info("K2HH calibration value /old %d %d %d/ new %d %d %d\n",acc->old_cal_data.x,acc->old_cal_data.y,acc->old_cal_data.z,acc->cal_data.x,acc->cal_data.y,acc->cal_data.z);

	/*if it is already enabled don't need to power on*/
...
static int k2hh_acc_disable(struct k2hh_acc_data *acc)
{
	if (atomic_cmpxchg(&acc->enabled, 1, 0)) {
		cancel_delayed_work_sync(&acc->input_poll_work);
#ifdef LGE_LPSR
		cancel_delayed_work_sync(&acc->input_md_poll_work);
#endif
		k2hh_acc_device_power_off(acc);
	}
...
#ifdef LGE_LPSR
static int k2hh_acc_md_enable(struct k2hh_acc_data *acc)
{
	...
	acc->resume_state[RES_CTRL2] |= 0x02;
	buf[0] = CTRL2;
	buf[1] = acc->resume_state[RES_CTRL2]  ; // need to tune
	err = k2hh_acc_i2c_write(acc, buf, 1);

	acc->resume_state[RES_INT_CFG1] |= (ALL_ZEROES | INT_CFG1_ZHIE |  INT_CFG1_YHIE |  INT_CFG1_XHIE ) ;
	buf[0] = INT_CFG1;
	buf[1] = acc->resume_state[RES_INT_CFG1]  ; // need to tune

	err = k2hh_acc_i2c_write(acc, buf, 1);

	/* read reference.. or 6 bytes in raw may be fater... */
	buf[0] = REF_XL;
	k2hh_acc_i2c_read(acc,&buf[0],1);
	buf[0] = REF_YL;
	k2hh_acc_i2c_read(acc,&buf[0],1);
	buf[0] = REF_ZL;
	k2hh_acc_i2c_read(acc,&buf[0],1);

	/* read interrupt status bit once to clear interrupt */
	buf[0] = INT_SRC1;
	k2hh_acc_i2c_read(acc,&buf[0],1);
	return 0;
}
static int k2hh_acc_md_disable(struct k2hh_acc_data *acc)
{
	int err = 0;
	u8 buf[2];
...
#ifdef LGE_LPSR
static ssize_t attr_get_enable_lpsr(struct device *dev,
			       struct device_attribute *attr, char *buf)
{
	struct k2hh_acc_data *acc = dev_get_drvdata(dev);
	int val = atomic_read(&acc->enable_state); // return enable_state
	return sprintf(buf, "%d\n", val);
}

static ssize_t attr_set_enable_lpsr(struct device *dev,
			       struct device_attribute *attr,
			       const char *buf, size_t size)
{
	struct k2hh_acc_data *acc = dev_get_drvdata(dev);
	...
	if (val) {
		dev_info(dev, "Enable MD");
		k2hh_acc_enable(acc);
		atomic_set(&acc->md_sample_count, MAX_MD_SAMPLE_COUNT);

		k2hh_acc_md_enable(acc);
		atomic_set (&acc->enable_state, atomic_read(&acc->enable_state)
				| ENABLE_MD);

		schedule_delayed_work(&acc->input_md_poll_work,
				msecs_to_jiffies(MD_REPORT_INTERVAL_MS));
	} else {
		dev_info(dev, "Disable MD");
		k2hh_acc_md_disable(acc);
		atomic_set(&acc->enable_state,
				atomic_read(&acc->enable_state) & ~ENABLE_MD);
	}
	// all functions are disabled
	if (atomic_read(&acc->enable_state) == 0) {
		dev_info(dev, "Disable acc");
			       ...
			       const char *buf, size_t size)
{
	...
	if (val) {
		dev_info(dev, "Enable Normal");
		k2hh_acc_enable(acc);
		atomic_set (&acc->enable_state, atomic_read(&acc->enable_state)
				| ENABLE_NORMAL);
		schedule_delayed_work(&acc->input_poll_work,
				msecs_to_jiffies(acc->pdata->poll_interval));
	} else {
		dev_info(dev, "Disable normal streaming");
		atomic_set(&acc->enable_state,
				atomic_read(&acc->enable_state) & ~ENABLE_NORMAL);
	}
...
static ssize_t attr_get_offset_backup(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct k2hh_acc_data *k2hh = dev_get_drvdata(dev);
	return sprintf(buf, "Read Calibrated Data old %d %d %d / new %d %d %d\n", k2hh->old_cal_data.x,k2hh->old_cal_data.y,k2hh->old_cal_data.z,k2hh->cal_data.x,k2hh->cal_data.y,k2hh->cal_data.z);
}
...
static ssize_t attr_set_threshx1(struct device *dev,
		struct device_attribute *attr,	const char *buf, size_t size)
{
	write_reg(dev, buf, INT_THSX1, INT1_THRESHOLD_MASK, RES_INT_THSX1);
	return size;
}
...
static ssize_t attr_set_threshy1(struct device *dev,
		struct device_attribute *attr,	const char *buf, size_t size)
{
	write_reg(dev, buf, INT_THSY1, INT1_THRESHOLD_MASK, RES_INT_THSY1);
	return size;
}
...
static ssize_t attr_set_threshz1(struct device *dev,
		struct device_attribute *attr,	const char *buf, size_t size)
{
	write_reg(dev, buf, INT_THSZ1, INT1_THRESHOLD_MASK, RES_INT_THSZ1);
	return size;
}
...
#ifdef LGE_CHECK_INTERRUPT_1
	/* A brief check routine for verifying irq */
	u8 tmp[2];

	tmp[0] = CTRL3;
	tmp[1] = 0x01; /* Set DRDY for immediate interrupt PIN check */
	k2hh->int_check = false;
	k2hh_acc_i2c_write(k2hh, tmp, 1);
	pr_info("Set to occure interrupt\n");

	msleep(100);
	if (k2hh->int_check == false) {
            tmp[0] = CTRL3;
	    tmp[1] = 0x00; /*Unset DRDY*/
	    k2hh_acc_i2c_write(k2hh, tmp, 1);
	    k2hh->int_check = true;
	    pr_info("Check interrupt failed %d\n", k2hh->int_check);
	    return -EINVAL;
	}

	pr_info("Check interrupt success %d\n", k2hh->int_check);
#endif
...
static void k2hh_acc_input_poll_work_func(struct work_struct *work)
{
	...
	if (atomic_read(&acc->enable_state) & ENABLE_NORMAL)  // LGE check enable_state instead
		schedule_delayed_work(&acc->input_poll_work,
				msecs_to_jiffies(acc->pdata->poll_interval));
...
#ifdef LGE_LPSR
static void k2hh_acc_input_md_poll_work_func(struct work_struct *work)
{
	struct k2hh_acc_data *acc;

	acc = container_of((struct delayed_work *) work,
			struct k2hh_acc_data, input_md_poll_work);

	k2hh_acc_report_triple_md(acc);

	mutex_lock(&acc->lpsr_lock);
	atomic_dec(&acc->md_sample_count);
	if (atomic_read(&acc->enable_state) & ENABLE_MD &&
			(atomic_read(&acc->md_sample_count) > 0 )) {
		schedule_delayed_work(&acc->input_md_poll_work,
				msecs_to_jiffies(MD_REPORT_INTERVAL_MS));
	}
	mutex_unlock(&acc->lpsr_lock);
...
#ifdef LGE_LPSR
		{"int_thr_x",       &pdata->int_thr_x,      DT_SUGGESTED,       DT_U32,     DEFAULT_INT_THR},
		{"int_thr_y",       &pdata->int_thr_y,      DT_SUGGESTED,       DT_U32,     DEFAULT_INT_THR},
		{"int_thr_z",       &pdata->int_thr_z,      DT_SUGGESTED,       DT_U32,     DEFAULT_INT_THR},
		{"int_dur",         &pdata->int_dur,        DT_SUGGESTED,       DT_U32,     DEFAULT_INT_DUR},
#endif
		{NULL,              NULL,                   0,                  0,          0},
	};
...
static int k2hh_store_Calibration_data(struct device *dev)
{
	...
	int fd_offset_x, fd_offset_y, fd_offset_z;
	unsigned char old_offset_src[16];
	int old_fd_offset;
	struct k2hh_acc_data *k2hh = dev_get_drvdata(dev);
	mm_segment_t old_fs = get_fs();
	...
	memset(old_offset_src,0x00,sizeof(old_offset_src));
	old_fd_offset = sys_open("/sns/old_offset.dat",O_WRONLY|O_CREAT, 0666);
	...
	sprintf(old_offset_src, "%d,%d,%d", k2hh->old_cal_data.x,k2hh->old_cal_data.y,k2hh->old_cal_data.z);
	...
	if ((sys_write(fd_offset_x, offset_x_src, sizeof(offset_x_src)) < 0)
		...
		return -EINVAL;
	if (sys_write(old_fd_offset, old_offset_src, sizeof(old_offset_src)) < 0)
		return -EINVAL;
	...
	sys_close(old_fd_offset);
		sys_chmod("/sns/old_offset.dat", 0664);

	set_fs(old_fs);
	//pr_info("K2HH calibration data :old %d %d %d/new %d %d %d\n",k2hh->old_cal_data.x,k2hh->old_cal_data.y,k2hh->old_cal_data.z,k2hh->cal_data.x,k2hh->cal_data.y,k2hh->cal_data.z);
#ifdef DEBUG
...
static int k2hh_do_calibration(struct device *dev, char *axis)
{
	...
	backup_x = k2hh->cal_data.x;
	backup_y = k2hh->cal_data.y;
	backup_z = k2hh->cal_data.z;
	...
	k2hh->cal_data.x = sum[0] / CALIBRATION_DATA_AMOUNT;  /* k2hh(16bit) */
	k2hh->cal_data.y = sum[1] / CALIBRATION_DATA_AMOUNT;  /* k2hh(16bit) */

	if (sum[2] >= 0) {
		k2hh->cal_data.z = (sum[2] / CALIBRATION_DATA_AMOUNT) - 8192; /* k2hh(16bit) 16384 @2g range */
	} else {
        /*[LGE] return cal fail when the phon screen facing down*/
        dev_err(dev, "Put the phone screen facing up");
        atomic_set(&k2hh->fast_calib_rslt, FLAT_FAIL);
        return -EINVAL;
	//k2hh->cal_data.z = (sum[2] / CALIBRATION_DATA_AMOUNT) + 8192; /* k2hh(16bit) 16384 @2g range */
	}
	k2hh->old_cal_data.x = backup_x;
	k2hh->old_cal_data.y= backup_y;
	k2hh->old_cal_data.z = backup_z;
...
#ifdef LGE_LPSR
static DEVICE_ATTR(enable_lpsr, S_IRUGO|S_IWUSR |S_IWGRP, attr_get_enable_lpsr, attr_set_enable_lpsr);
#endif
...
#endif
static DEVICE_ATTR(offset_backup, S_IRUGO|S_IWUSR |S_IWGRP,    attr_get_offset_backup, NULL);
static struct attribute *acc_sysfs_attrs[] = {
	&dev_attr_enable.attr,
	&dev_attr_enable_lpsr.attr,
...
static int k2hh_acc_input_init(struct k2hh_acc_data *acc)
{
	...
	INIT_DELAYED_WORK(&acc->input_poll_work, k2hh_acc_input_poll_work_func);
...
#ifdef LGE_LPSR
	INIT_DELAYED_WORK(&acc->input_md_poll_work, k2hh_acc_input_md_poll_work_func);
	acc->input_dev_md = input_allocate_device();
	if (!acc->input_dev_md) {
		err = -ENOMEM;
		dev_err(&acc->client->dev, "input device allocation failed\n");
		goto err0;
	...
	acc->input_dev_md->open = k2hh_acc_input_open;
	acc->input_dev_md->close = k2hh_acc_input_close;
	/*acc->input_dev->name = K2HH_ACC_DEV_NAME;*/
	acc->input_dev_md->name = "lpsr";
	acc->input_dev_md->uniq = "k2hh";
	acc->input_dev_md->dev.init_name = LGE_LPSR_NAME;

	input_set_drvdata(acc->input_dev_md, acc);
	set_bit(EV_ABS, acc->input_dev_md->evbit);

	input_set_abs_params(acc->input_dev_md, ABS_X, G_MIN, G_MAX, FUZZ, FLAT);
	input_set_abs_params(acc->input_dev_md, ABS_Y, G_MIN, G_MAX, FUZZ, FLAT);
	input_set_abs_params(acc->input_dev_md, ABS_Z, G_MIN, G_MAX, FUZZ, FLAT);

	err = input_register_device(acc->input_dev_md);
	if (err) {
		dev_err(&acc->client->dev,
				"unable to register input device %s\n",
				acc->input_dev_md->name);
		goto err1;
	}
#endif
	return 0;
...
#ifdef LGE_LPSR
	input_free_device(acc->input_dev_md);
#endif
err0:
	return err;
}
...
static void k2hh_acc_input_cleanup(struct k2hh_acc_data *acc)
{
	...
	input_free_device(acc->input_dev);
#ifdef LGE_LPSR
	input_unregister_device(acc->input_dev_md);
	input_free_device(acc->input_dev_md);
#endif
}
...
#ifdef LGE_LPSR
	err = sysfs_create_group(&acc->input_dev_md->dev.kobj, &acc_attribute_group);
	if (err < 0) {
		dev_err(&client->dev,
		   "device K2HH_ACC_DEV_NAME sysfs register failed\n");
		goto err_input_cleanup;
...
#ifdef LGE_LPSR
	mutex_init(&acc->lpsr_lock);
	atomic_set(&acc->md_sample_count, 0);
#endif
	if (acc->pdata->gpio_int1 >= 0) {
...
#endif
	atomic_set(&acc->enabled, 0);
	atomic_set(&acc->enable_state, 0);
#ifdef LGE_CHECK_INTERRUPT_1
	acc->int_check = true;
	enable_irq(acc->irq1);
	dev_err(&client->dev, "%s:init_check = %d enable_irq done\n", K2HH_ACC_DEV_NAME, acc->int_check);
#endif
	k2hh_acc_device_power_off(acc);
...
static int k2hh_acc_resume(struct i2c_client *client)
{
	...
	if (acc->on_before_suspend) {
		...
		if (err) {
			dev_err(&acc->client->dev, "Resume Enabling Error\n");
			return err;
		}
		if (atomic_read(&acc->enable_state) & ENABLE_MD) {
			k2hh_acc_md_enable(acc);
		...
		if (atomic_read(&acc->enable_state) & ENABLE_NORMAL) {
			schedule_delayed_work(&acc->input_poll_work,
				msecs_to_jiffies(acc->pdata->poll_interval));
		}
	}
...
static void async_sensor_init(void *data, async_cookie_t cookie)
{
	pr_info("%s accelerometer driver: init\n",
						K2HH_ACC_DEV_NAME);
	i2c_add_driver(&k2hh_acc_driver);
	return;
}
