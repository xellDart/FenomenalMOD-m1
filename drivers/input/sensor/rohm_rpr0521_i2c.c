...
//#define LIGHT_SENSOR_INTERRUPT_MODE
...
#ifndef LIGHT_SENSOR_INTERRUPT_MODE
...
#ifdef LIGHT_SENSOR_INTERRUPT_MODE
    parg.intr = MODE_BOTH;
#else	
...
static int update_als_value(unsigned short new_als_data0, unsigned short new_als_data1)
{
	...
	ps_als_ginfo->als_data1 = new_als_data1;
...
#ifdef LIGHT_SENSOR_INTERRUPT_MODE
        if(new_als_data0 > ps_als_ginfo->config.init_data.alsth_low && new_als_data0 >=ps_als_ginfo->config.init_data.alsth_upper)
	    {
		    PINFO("ch0data(%d) > bright threshold(%d) als state is changed dark to bright",
	    ...
	    else if(new_als_data0 <= ps_als_ginfo->config.init_data.alsth_low && new_als_data0 < ps_als_ginfo->config.init_data.alsth_upper)
	    {
		    PINFO("ch0data(%d) < dark threshold(%d) als state is changed bright to dark",
	    ...
	    {
		    PINFO("als state is not changed. ch0data(%d)", new_als_data0);
	    }
#endif
    return 0;
}
...
static void ps_als_work_func(struct work_struct *work)
{
...
#ifdef LIGHT_SENSOR_INTERRUPT_MODE
    status = i2c_smbus_read_byte_data(ps_als_ginfo->client, 0x4A);
    PINFO("update_ps_and_als_value status = 0x%x\n",status);
#endif
    /* clear interrupt flag */
    gene_data.adr_reg = REG_INTERRUPT;
    gene_data.addr    = (unsigned char *)&read_intr;
	...
	if (pwr_st.ps_state == CTL_STANDALONE && ps_als_ginfo->is_interrupt) {
#ifdef LIGHT_SENSOR_INTERRUPT_MODE		
		if ((status & 0x80) == 0x80) {  // PS interrupt!
        ...
        } else if((status & 0x40) == 0x40) { //ALS interrupt!
            update_als_value(read_data_buf.als_data0, read_data_buf.als_data1);
        }
...
#ifdef LIGHT_SENSOR_INTERRUPT_MODE
	ps_als->config.init_data.intr = (PS_THH_BOTH_OUTSIDE | MODE_BOTH);
#else
