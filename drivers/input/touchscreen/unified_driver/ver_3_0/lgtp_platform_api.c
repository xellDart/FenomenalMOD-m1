...
#if defined(TOUCH_DEVICE_SN280H)
#define ADDR_SIZE 2
int Semisense_I2C_Read(struct i2c_client *client, u8 *cmd, u8 *data, u32 cmd_len, u32 len)
{
	u8 regValue[ADDR_SIZE] = {cmd[0], cmd[1]};
	int ret = 0;

	ret = i2c_read(client, regValue, cmd_len, data, len);
	if (ret == TOUCH_FAIL) {
		if (printk_ratelimit())
...
int Semisense_I2C_Write(struct i2c_client *client, u8 *cmd, u8 *data, u32 cmd_len, u32 len)
{
	u8 regValue[ADDR_SIZE] = {cmd[0], cmd[1]};
	int ret = 0;

	ret = i2c_write(client, regValue, cmd_len, data, len);
	if (ret == TOUCH_FAIL) {
		if (printk_ratelimit())
