...
static int MIT200_GetDataFormat(struct i2c_client *client, struct mit200_data_format *data_form)
{
	u8 reg_buf[2];
	u8 rbuf[10];
	...
	if(Mit200_proc4_I2C_Read(client, reg_buf, 2, rbuf, 6)<0){
...
static int MIT200_GetBufAddr(struct i2c_client *client, struct mit200_buf_addr *buf_addr)
{
	u8 reg_buf[2];
	u8 rbuf[10];
	...
	if(Mit200_proc4_I2C_Read(client, reg_buf, 2,rbuf, 2)<0){
...
/*
static int MIT200_ReadRMIBuffer(struct i2c_client *client, u8 size, u8 data_type_size, 
										u8 data_type_sign, u8 buf_addr_h, u8 buf_addr_l, u8 row_num, 
										u8 col_num, u8 buf_col_num, u8 rotate, u8 key_num)
{
*/
static int MIT200_ReadRMIBuffer(struct i2c_client *client, struct mit200_data_format *data_form, struct mit200_buf_addr *buf_addr)
{
...
static int  MIT200_PrintRawData(struct i2c_client *client, char *buf,int type)
{
	int col = 0;
	int row = 0;
	int ret = 0;
...
static int  MIT200_PrintIntensity(struct i2c_client *client, char *buf) {
	int col = 0;
	int row = 0;
	int ret = 0;
...
int MIT200_DoTest(struct i2c_client *client, u8 image_type){
  	int busy_cnt = 100;
	int wait_cnt = 50;
	u8 wbuf[8];
	...
	if (MIT200_GetDataFormat(client, data_form)) {
		printk(KERN_ERR "%s [ERROR] MIT200_GetDataFormat\n", __func__);
		goto ERROR;
	}

	if (MIT200_GetBufAddr(client, buf_addr)) {
		printk(KERN_ERR "%s [ERROR] MIT200_GetBufAddr\n", __func__);
		goto ERROR;
	}

	if(MIT200_ReadRMIBuffer(client, data_form, buf_addr)) {
		printk(KERN_ERR "%s [ERROR] MIT200_ReadRMIBuffer\n", __func__);
		goto ERROR;
	}
...
RETRY :
	...
	if (type == RAW_DATA_SHOW || type == RAW_DATA_STORE) {
		if (MIT200_DoTest(client, MIP_IMG_TYPE_RAWDATA) == TOUCH_FAIL) {
			TOUCH_ERR("getting raw data failed\n");
			printk(KERN_ERR "getting raw data failed\n");
			goto RETRY;
		}
	} else if (type == DELTA_SHOW) {
		if (MIT200_DoTest(client, MIP_IMG_TYPE_INTENSITY) == TOUCH_FAIL) {
				TOUCH_ERR("getting raw data failed\n");
				printk(KERN_ERR "getting raw data failed\n");
		}
	}
...
*/
	...
	switch(type) {
		case RAW_DATA_SHOW:
			...
			ret = MIT200_PrintRawData(client, buf, type);
			if (ret < 0) {
		...
		case RAW_DATA_STORE:
			...
			ret = MIT200_PrintRawData(client, read_buf, type);
			if (ret < 0) {
				TOUCH_ERR("fail to print raw data\n");
				goto error;
			}
		...
		case DELTA_SHOW:
			...
			ret = MIT200_PrintIntensity(client, buf);
			if (ret < 0) {
