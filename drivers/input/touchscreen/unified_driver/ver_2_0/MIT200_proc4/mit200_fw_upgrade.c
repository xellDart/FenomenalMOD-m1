...
static int mip_isc_program_page(struct melfas_ts_data *ts, int offset, const u8 *data, int length)
{
	...
	*/
	...
	if(Mit200_proc4_I2C_Write(ts->client, write_buf, (length + 6))){
...
static int mip_isc_erase_page(struct melfas_ts_data *ts, int offset)
{
	...
	mip_isc_program_page(ts, offset, data, ISC_PAGE_SIZE);
	if(ret != 0){
		TOUCH_ERR("mip_isc_program_page\n");
		goto ERROR;
	}

	TOUCH_LOG("[DONE] - Offset [0x%04X]\n",offset);
	return 0;
ERROR:
	return -1;
}
