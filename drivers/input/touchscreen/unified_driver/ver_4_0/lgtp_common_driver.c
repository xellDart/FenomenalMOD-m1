...
static void WqTouchInit(struct work_struct *work_init)
{
	...
	/* In Abnormal IC status and LPWG mode, Re-setting LPWG mode  */
...
/****************************************************************************
* Developer will use it to know call state from framework
****************************************************************************/
...
static ssize_t show_incoming_call_state(TouchDriverData *pDriverData, char *buf)
{
	...
	value = atomic_read(&pDriverData->incoming_call);
...
static ssize_t store_incoming_call_state(TouchDriverData *pDriverData, const char *buf, size_t count)
{
	...
	if (value >= INCOMING_CALL_IDLE && value <= INCOMING_CALL_OFFHOOK) {
		atomic_set(&pDriverData->incoming_call, value);
		pDeviceControlFunc->NotifyHandler(NOTIFY_CALL, value);
		TOUCH_LOG("%s : %s(%d)\n", __func__,
				incoming_call_str[value], value);
	} else {
...
//    #endif
    ...
    if(event == FB_EARLY_EVENT_BLANK)
	{
		if (*blank == FB_BLANK_POWERDOWN)
		{
		   ...
		   if(is_mfts_mode && !pDriverData->mfts_lpwg)
			   pDeviceControlFunc->Power(0);
		   else
           SetNextState(0);
           #endif
